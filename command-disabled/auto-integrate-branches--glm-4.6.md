...
!`git for-each-ref --format='%(refname:short)' refs/remotes/origin | sed '/^origin$/d; s#^origin/##' | grep -v ^integration\/ | grep -v ^fix\/ | grep -v ^merged\/ | grep -v ^pr\/ | grep -v ^merging\/ | grep -v ^obsolete\/ | grep -v ^failed\/ | grep -v ^wip\/ | grep -v '^dev$'`
