...
You **MUST** set the title of the current session to "integrations|integration/$1" ("integrations|" followed by the name of the integration branch) and **MUST** bookmark it using the `bookmark_current_session` tool. You **MUST NOT** forget to include the pipe character in the session title! Using this exact sche for the titles is **CRUCIAL** for compatibility with our CI tools! You **MUST** not add any additional prefix or suffix and must follow this format exactly for proper integration with our CI tools! 
...
**REMEMBER**: You **MUST** push the integration branch to origin after merging **EACH** branch!