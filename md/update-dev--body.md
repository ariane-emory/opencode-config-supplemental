...
!`baseone expand ~/.config/opencode/md/set-title-format.md "sync|Syncing dev with upstream/dev"`

**CRITICAL**: The local dev branch and origin/dev **MUST** be at the exact same commit hash as upstream/dev when you are done, this is not negotiable! You **MUST** NOT try to fix errors in upstream/dev, if upstream/dev has errors in it we need those errors in dev too!r
...
**FINALLY**, while still on the dev branch, run `bun install` to ensure that all packages are up to date. 
